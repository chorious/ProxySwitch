...
namespace proxy
{
        ...
        public netlib::log::logger<socks_local_router>
    {
        ...
        /**
         * @brief Entry for PID-based routing, storing the proxy ID and process creation time
         *        for reuse protection.
         */
        struct pid_route_entry
        {
            size_t proxy_id{};
        ...
        /**
         * @brief Maps process IDs to their proxy route entries.
         */
         ...
         *        Marked mutable so read-only (const) query methods can acquire
         *        shared locks for thread-safe access.
         */
    ...
    public:
         ...
         * @param proxy_id the ID of the proxy server to associate with the process
         * @param creation_time the process creation time for reuse protection
         * @return True if the association was successful, False otherwise
         */
        bool associate_process_id_to_proxy(const DWORD pid, const size_t proxy_id, const FILETIME& creation_time)
        {
            {
                std::scoped_lock lock(lock_);
                ...
                {
                    NETLIB_LOG(log_level::error,
                        "associate_process_id_to_proxy: proxy index is out of range!");
                    return false;
                }

                try
                {
                    pid_to_proxy_[pid] = pid_route_entry{ proxy_id, creation_time };
                }
                catch (const std::exception& e) {
                    NETLIB_LOG(log_level::error, "Exception associating process ID to proxy: {}", e.what());
                    return false;
                }
                catch (...) {
                    NETLIB_LOG(log_level::error, "Unknown exception associating process ID to proxy.");
                    return false;
                }
            }

            // Force the packet filter to re-resolve process identities so the new
            // PID route is visible immediately (bypass cache recovery).
        ...
        /**
         * Removes a process ID from the PID-based routing table. This function is thread-safe.
         * @param pid the process ID to remove
         * @return True if the removal was successful or the PID was not present, False on error
         */
        bool remove_process_id(const DWORD pid)
        {
            std::scoped_lock lock(lock_);

            try
            {
                pid_to_proxy_.erase(pid);
            }
            catch (const std::exception& e) {
                NETLIB_LOG(log_level::error, "Exception removing process ID from proxy: {}", e.what());
                return false;
            }
            catch (...) {
                NETLIB_LOG(log_level::error, "Unknown exception removing process ID from proxy.");
                return false;
            }
        ...
        /**
         * Clears all PID-based routing entries. This function is thread-safe.
         */
        void clear_pid_routes()
        {
            std::scoped_lock lock(lock_);
            pid_to_proxy_.clear();
        }
        ...
        /**
         * Refreshes the process lookup tables to clear stale bypass caches.
         * Call this after adding PID routes to ensure immediate effect.
         */
        void refresh_process_lookups()
        {
    ...
    private:
         ...
         * @return true if a PID route exists, false otherwise.
         */
        bool has_pid_route(const DWORD pid) const
        {
            std::shared_lock lock(lock_);
            return pid_to_proxy_.find(pid) != pid_to_proxy_.end();
        }

        /**
         * @brief Validates that a PID's current creation time matches the expected value.
         * @param pid The process ID to validate.
         * @param expected The expected creation time from the route entry.
         * @param out_actual Optional output for the actual creation time.
         * @return true if the PID exists and its creation time matches expected, false otherwise.
         */
        static bool validate_pid_creation_time(const DWORD pid, const FILETIME& expected, FILETIME& out_actual) noexcept
        {
            const HANDLE h = OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION, FALSE, pid);
            if (!h) return false;

            FILETIME create{}, exit{}, kernel{}, user{};
            const bool ok = GetProcessTimes(h, &create, &exit, &kernel, &user) != 0;
            CloseHandle(h);

            if (ok) out_actual = create;
            return ok &&
                   create.dwLowDateTime == expected.dwLowDateTime &&
                   create.dwHighDateTime == expected.dwHighDateTime;
        }

        /**
         * @brief Removes stale PID route entries whose process no longer exists
         *        or whose creation time has changed (PID reuse).
         *        Must be called with no locks held (acquires lock_ internally).
         */
        ...
        std::optional<uint16_t> get_proxy_port_tcp(const std::shared_ptr<iphelper::network_process>& process)
        {
            ...
            // 1) PID-based routing takes precedence over app-name rules
            if (const auto it = pid_to_proxy_.find(process->id); it != pid_to_proxy_.end())
            {
                FILETIME actual{};
        ...
        void process_resolve_thread_proc()
        {
            ...
            while (true)
            {
                    ...
                    now - last_sweep >= maintenance_interval)
                {
                    ...
                    // Evict stale PID route entries whose process no longer exists
                    // or whose creation time has changed (PID reuse).
                    cleanup_stale_pid_routes();
                }
