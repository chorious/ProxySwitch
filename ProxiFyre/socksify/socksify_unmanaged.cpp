 ...
 * @return True if association was successful, false otherwise.
 */
bool socksify_unmanaged::associate_process_id_to_proxy(const DWORD process_id,
    const LONG_PTR proxy_id, const FILETIME& creation_time) const
{
    return proxy_->associate_process_id_to_proxy(process_id, static_cast<size_t>(proxy_id), creation_time);
}

/**
 * @brief Removes a process ID from the PID-based routing table.
 * @param process_id The process ID to remove.
 * @return True if removal was successful, false otherwise.
 */
bool socksify_unmanaged::remove_process_id(const DWORD process_id) const
{
    return proxy_->remove_process_id(process_id);
}

/**
 * @brief Clears all PID-based routing entries.
 */
void socksify_unmanaged::clear_pid_routes() const
{
    proxy_->clear_pid_routes();
}
