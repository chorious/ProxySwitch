...
/// <summary>
/// Associates a process ID with a specific proxy instance.
/// </summary>
/// <param name="pid">The process ID to associate.</param>
/// <param name="proxy">The proxy handle.</param>
/// <param name="createdAtFileTimeUtc">The process creation time as a FILETIME (100-ns intervals since 1601-01-01 UTC).</param>
/// <returns>True if association was successful, otherwise false.</returns>
bool Socksifier::Socksifier::AssociateProcessIdToProxy(UInt32 pid, IntPtr proxy, Int64 createdAtFileTimeUtc)
{
    if (!unmanaged_ptr_)
        return false;

    FILETIME ft;
    ft.dwLowDateTime = static_cast<DWORD>(createdAtFileTimeUtc);
    ft.dwHighDateTime = static_cast<DWORD>(createdAtFileTimeUtc >> 32);

#if _WIN64
    return unmanaged_ptr_->associate_process_id_to_proxy(pid, proxy.ToInt64(), ft);
#else
    return unmanaged_ptr_->associate_process_id_to_proxy(pid, proxy.ToInt32(), ft);
#endif
}

/// <summary>
/// Removes a process ID from the PID-based routing table.
/// </summary>
/// <param name="pid">The process ID to remove.</param>
/// <returns>True if removal was successful, otherwise false.</returns>
bool Socksifier::Socksifier::RemoveProcessId(UInt32 pid)
{
    if (!unmanaged_ptr_)
        return false;
    return unmanaged_ptr_->remove_process_id(pid);
}

/// <summary>
/// Clears all PID-based routing entries.
/// </summary>
void Socksifier::Socksifier::ClearPidRoutes()
{
    if (!unmanaged_ptr_)
        return;
    unmanaged_ptr_->clear_pid_routes();
}